...
function voyageList(pageNum){
    $.ajax({
        ...
        type:'post',
        data: {"resource.vid":$("#idQuery").val(),
        	"resource.productStatus":$("#statusQuery").val(),
        	"resource.title":$("#titleQuery").val(),
        	"resource.productType":$("#productTypeQuery").val(),
        	'resource.providerName':$("#providerNameQuery").val(),
        	"resource.code":$("#codeQuery").val(),
        	'resource.travelDate':$("#inpstart").val(),
			'resource.deadline':$("#inpend").val(),
...
function rendList(data){
    ...
    for(var i = 0; i < datas.length; i++){
        ...
        var logo = '';
        var productType = datas[i].productType;
        if(productType ==21){
        ...
        list = list +
            '<tr>';
            if(productType == 21){
               list = list+'<td><a href="'+getContextPath()+'/manager/resource/voyage.jsp?id='+datas[i].id+'&code='+code+'&cruiseId='+cruiseId+'&days='+days+'&title='+escape(title)+'&productType='+productType+'" target="right">'+datas[i].id+'</a></td>'+
                    '<td style="text-align:center;vertical-align:middle;">'+datas[i].versionTitle+'</td>'+
                     logo+
                     '<td >'+datas[i].providerName+'</td>' +
        ...
        list = list+
        '<td><a href="'+getContextPath()+'/operator/travel/cruise-detail.jsp?productType='+datas[i].productType+'&code='+datas[i].resCode+'&cruiseId='+datas[i].cruise_id+'" target="right">'+datas[i].resCode+'</a></td>'
        + '<td>';
        	if(productType == 21){
        		list = list+'<a href="'+getContextPath()+'/manager/resource/addVoyage.jsp?id='+datas[i].id+'&code='+datas[i].resCode+'&cruiseId='+datas[i].cruise_id+'&days='+datas[i].days+'&crusieLimit='+datas[i].crusie_limit+'&title='+escape(datas[i].versionTitle)+'&productType='+productType+'&type=version">编辑班期</a><br>';
        	}else{
