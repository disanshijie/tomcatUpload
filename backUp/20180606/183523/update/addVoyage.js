...
function fillGoodsData(housesInfo){
	...
	for(var i=0; i<housesInfo.length; i++) {
		...
		if(housesInfo[i].shipping_space_id == 0000003){//如果是挂舱
			$("#addSpaceName").val(housesInfo[i].house_type);//产品名称（shippingSpace）
			$("#addSpaceTotalNum").val(housesInfo[i].total_num);//产品数量（totalNum）
			$("#addSpaceTotalNum").next("input").val(housesInfo[i].total_num);//产品数量（totalNum）
			$("#addSpaceNum").val(housesInfo[i].house_num);//产品数量（totalNum）
			$("#addSpaceNum").next("input").val(housesInfo[i].house_num);//产品数量（totalNum）
			...
			$(".editAddSpaceGoods").show();
			$(".saveAddSpaceGoods").hide();
		}else if(housesInfo[i].shipping_space_id == 0000002){//如果是大巴
			$("#bus").val(housesInfo[i].customer_price);//产品价格（traderPrice umPrice customerPrice）
			$("#busId").val(housesInfo[i].id);//产品价格（traderPrice umPrice customerPrice）
			$(".editBusGoods").show();
			$(".saveBusGoods").hide();
		}else if(housesInfo[i].shipping_space_id == 0000001){//如果是保险
			$("#insurance").val(housesInfo[i].customer_price);//产品价格（traderPrice umPrice customerPrice）
			$("#insuranceId").val(housesInfo[i].id);//产品价格（traderPrice umPrice customerPrice）
			$(".editInsuranceGoods").show();
			$(".saveInsuranceGoods").hide();
			
		}else{
			list = list +
			'<li style="margin-bottom: 10px;">'+
		 	'<div class="cabin-type-name "><input class="form-control goodName" value="'+housesInfo[i].house_type+'" type="text"></div>'+
		 	'<div class="cabin-name "><input class="form-control goodsTotalNum" value="'+housesInfo[i].total_num+'" type="text"><input value="'+housesInfo[i].total_num+'" type="text" style="display:none;"></div>'+
		 	'<div class="cabin-name "><input class="form-control goodsNum" value="'+housesInfo[i].house_num+'" type="text" readonly="true"><input value="'+housesInfo[i].house_num+'" type="text" style="display:none;"></div>'+
		 	'<div class="cabin cabinNum "><input class="form-control goodsPrice" value="'+housesInfo[i].customer_price+'" type="text"></div>'+
		 	'<div class="cabin cabinNum " style="display:none"><input class="form-control goodsId" value="'+housesInfo[i].id+'" type="text"></div>'+
	...
	$(".goodsTotalNum").keyup(
			function(){  
				 ...
				 var goodsNumOld = Number($(this).parents(".cabin-name").next().find("input:eq(1)").val());//老的剩余 
				 if(goodsNumOld == "" || goodsNumOld ==null){
					 
				 }
				  var goodsTotalNumOld = $(this).next("input").val();//老总间
				  var goodsTotalNum = Number($(this).val());//新的总间数
				  goodsTotalNum = isNaN(goodsTotalNum)?0:goodsTotalNum;
				  var diffNum = goodsTotalNum - Number(goodsTotalNumOld);//新旧差额间数
				  
				  var goodsNum = goodsNumOld+diffNum;
				  $(this).parents(".cabin-name").next().find("input:eq(0)").val(goodsNum);
		});
}
...
function saveGoods(obj){
	var cabinId = "0";
	var shippingSpaceId = "0000001";
	var goodsName=$(obj).parents("li").find("div:eq(0)").find("input:eq(0)").val();//名字
	var goodsTotalNum=$(obj).parents("li").find("div:eq(1)").find("input:eq(0)").val();//总量
	var goodsPrice=$(obj).parents("li").find("div:eq(3)").find("input:eq(0)").val();//价格
	var reg=/^\d{1,5}$/;
	...
	if(goodsName == "大巴"){
		goodsTotalNum = 999;
		goodsNum = 999;
		shippingSpaceId = "0000002";
	}
	//挂舱
	if(goodsName == "挂舱"){
		shippingSpaceId = "0000003";
	...
	$.ajax({
		...
		data:{
			...
			'cruiseHouse.houseNum':goodsTotalNum,
			...
			'cruiseHouse.personNum':goodsTotalNum
...
//	var shippingSpaceId = 0//添加附加产品（）
	var goodsName = $(this).parents("li").find("div:eq(0)").find("input:eq(0)").val();//产品名称（shippingSpace）
	var goodsTotalNum = $(this).parents("li").find("div:eq(1)").find("input:eq(0)").val();//产品数量（totalNum）
	var goodsNum = $(this).parents("li").find("div:eq(2)").find("input:eq(0)").val();//产品数量（totalNum）
	var goodsPrice = $(this).parents("li").find("div:eq(3)").find("input:eq(0)").val();//产品价格（traderPrice umPrice customerPrice）
	var cruiseHouseId = $(this).parents("li").find("div:eq(4)").find("input:eq(0)").val();//产品价格（traderPrice umPrice customerPrice）
	var reg=/^\d{1,5}$/;
	var regFloat = /^[0-9]+(.[0-9]{2})?$/;
	var regTotalNmu=/^\+?[0-9][0-9]*$/;
	
	if(goodsName == "保险" || goodsName == "大巴"){
		goodsTotalNum = 999;
		goodsNum = 999;
		if(goodsPrice==null || goodsPrice=='' || (!reg.test(goodsPrice) && !regFloat.test(goodsPrice))) {
			Jalert('产品价格为空或不合法');
...
function submitGoods() {
	...
	var goodsNum = goodsTotalNum;//产品数量（totalNum）
